...
void __my_log_stderr_logger(const struct __android_log_message* log_message) {
  struct tm now;
  ...
  if (log_message->file != nullptr) {
    ...
    // fprintf(stderr, "%s %5d %5 %c %s:" PRIu64 " %s\n",
    //     timestamp, getpid(), tid, priority_char, log_message->tag ? log_message->tag : "nullptr",
    //     log_message->message);

    // fprintf(stderr, "%s %5d %5" PRIu64 " %c %s: %s\n",
    //     timestamp, getpid(), tid, priority_char, log_message->tag, log_message->message);
    
    char buffer[1024];
    sprintf(buffer, "%s %5d %5" PRIu64 " %c %s: %s\n",
        timestamp, getpid(), tid, priority_char, log_message->tag, log_message->message);
      
    
    //if (byd_auto_hal::g_autohalLogWriter != nullptr) {
      byd_auto_hal::g_autohalLogWriter.add(buffer);
      fprintf(stderr, "%s", buffer);
    //}
    // else {
    //   fprintf(stderr, "日志记录失败，没有被初始化 %s", buffer);
    // }
  }
...
void __android_log_write_log_message(__android_log_message* log_message) {
  ...
  __my_log_stderr_logger(log_message);
  // 将日志输出到缓冲区
  /*
  if(logWriter != nullptr) {
    logWriter->pBuffer->push(logStr);
  }
  else {
    return;
  }
...
/* LogWriter初始化与结束，函数的实现方式

namespace byd_auto_hal {

autoHalLogWriterInit() {
    AutoHalLogWriter* logWriter = new AutoHalLogWriter();
}
